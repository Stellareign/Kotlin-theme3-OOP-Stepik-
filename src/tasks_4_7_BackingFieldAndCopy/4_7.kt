...
class Employee(val id: Int, val name: String) {
    ...
    fun updateTaskStatus(taskId: Int, newStatus: String) {
        val task = tasks.find { it.id == taskId }
        if (task != null) {
           val copyTask = task.copy(status = newStatus) // Прямое изменение, требующее изменения на использование copy()
            _archivedTasks.add(task) // архивируем старую задачу
            _tasks[_tasks.indexOf(task)] = copyTask // сохраняем новую задачу
    ...
    fun changeTaskAssignee(taskId: Int, newAssignee: String) {
        val task = tasks.find { it.id == taskId }
        if (task != null) {
           val taskNewAssignee = task.copy(assignedTo = newAssignee) // Прямое изменение, требующее изменения на использование copy()
            _archivedTasks.add(task)
            _tasks[_tasks.indexOf(task)] = taskNewAssignee
    ...
    fun updateTaskPriority(taskId: Int, newPriority: String) {
        val task = tasks.find { it.id == taskId }
        if (task != null) {
            val taskNewPriority = task.copy(priority = newPriority)  // Прямое изменение, требующее изменения на использование copy()
            _archivedTasks.add(task)
            _tasks[_tasks.indexOf(task)] = taskNewPriority
    ...
    fun modifyTaskDetails(taskId: Int, newTitle: String, newDescription: String) {
        val task = tasks.find { it.id == taskId }
        if (task != null) {
            val taskNewDetailes = task.copy(title =newTitle, description = newDescription)  // Прямое изменение, требующее изменения на использование copy()
            _archivedTasks.add(task)
            _tasks[_tasks.indexOf(task)] = taskNewDetailes
